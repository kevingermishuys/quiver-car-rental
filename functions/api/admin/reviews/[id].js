// POST /api/admin/reviews/:id — approve, reject, or delete a review
...
export async function onRequestPost({ request, env, params }) {
  ...
  if (body.action === "delete") {
    await env.REVIEWS_DB.prepare("DELETE FROM reviews WHERE id = ?").bind(id).run();
    return json({ ok: true });
  }
  ...
  await env.REVIEWS_DB.prepare("UPDATE reviews SET status = ? WHERE id = ?").bind(status, id).run();
  return json({ ok: true });
}
